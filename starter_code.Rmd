...
```{r}
# Using 1% of the data using initial_split()

set.seed(10)

n_split <- initial_split(clean, prop = 1/100)

sample_data <- training(n_split)
...
logistic_reg() %>%
  set_engine("glm") %>%
  fit(dvote ~ gender + party + state + office, data = sample_data) %>%
  predict(new_data = testing(n_split)) %>%
  bind_cols(testing(n_split)) %>%
  metrics(truth = dvote, estimate = .pred_class) %>%
  gt() %>% 
